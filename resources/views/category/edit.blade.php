                        ...
                        <div class="mb-4 p-4 rounded-lg bg-red-500/10 border border-red-500/30">
                            <ul class="space-y-1">
                                @foreach ($errors->all() as $error)
                                    <li class="text-xs font-bold list-disc ml-4" style="color: #f87171 !important;">{{ $error }}</li>
                                @endforeach
                            </ul>
                        </div>
                    @endif

                    <form action="{{ route('category.update', $category->id) }}" method="POST" class="space-y-5" novalidate>
                        @csrf
                        @method('PUT')

                        <div class="space-y-2">
                            <label for="name" class="block text-sm font-medium text-gray-300">Category</label>
